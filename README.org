...
For statistics nerds (🤓), the actual R code that generated the MFA on which the dashboard relies, using the [[http://factominer.free.fr/factomethods/multiple-factor-analysis.html][FactoMineR]] R package, can be found in the =R/mfa.R= file.
